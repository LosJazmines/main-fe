...
export class DrawerComponent implements OnChanges, OnInit, OnDestroy {
  ...
  @Output() readonly modeChanged: EventEmitter<drawerMode> =
    new EventEmitter<drawerMode>();
  @Output() readonly openedChanged: EventEmitter<boolean> =
    new EventEmitter<boolean>();
  @Output() readonly positionChanged: EventEmitter<drawerPosition> =
    new EventEmitter<drawerPosition>();
  ...
  private _hideOverlay(): void {
    ...
    this._player.onDone(() => {
      ...
      if (this._overlay) {
        ...
        // Get the parent node
        const parentNode = this._overlay.parentNode;

        // Check if the parent node exists before trying to remove the overlay
        if (parentNode) {
          // Remove the overlay
          parentNode.removeChild(this._overlay);
        }
        // this._overlay = null;
      }
    });
